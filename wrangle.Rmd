...
```{r}
ggplot(data_long_pp, aes(x = ratings, fill = training)) +
...
```{r}
ggplot(data_long_pp2, aes(x=fct_rev(training))) +
  geom_bar(aes(fill=fct_rev(ratings)), position = "fill") +
  coord_flip() +
  # theme(element_text(legend.title = "rating")) +
  # scale_fill_brewer(palette = "Accent") +
  facet_grid(training_type~image_type~dv)
```
...
```{r}
ggplot(subset(data_long_pp2, image_type = sorolla), aes(x=fct_rev(training))) +
  geom_bar(aes(fill=fct_rev(ratings)), position = "fill") +
  coord_flip() +
  # theme(element_text(legend.title = "rating")) +
  scale_fill_brewer(palette = "Accent") +
...
```{r}
# without training_type
p5.6 <- ggplot(datag_long_g_summary, 
              aes(x=condition, y=mean, fill=condition)) + 
...
ggsave ("figures/ratings_g.jpeg",plot = p5.6, width = 20, height = 13, units = "in", dpi = 300)

# with training_type
p5.7 <- ggplot(datag_long_g_training, 
              aes(x=condition, y=mean, fill=condition)) + 
...
ggsave ("figures/ratings_g_training_type.jpeg",plot = p5.7, width = 20, height = 13, units = "in", dpi = 300)
```
